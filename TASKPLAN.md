...
### Definition of Done
- [x] Website bisa diakses via URL publik — **live di https://dzakydev.my.id** (Vercel project `personal-portofolio-76js`, team `jaee`, auto-deploy dari GitHub `monn01/personal-portofolio` branch `main`). Langkah deploy lengkap dipindah ke `DEPLOY.md` (dieksekusi user, tuntas)
...
- [x] Login admin tetap berfungsi di production — akun admin production sudah dibuat (Tahap 7 `DEPLOY.md`), terkonfirmasi bisa login di domain live
...
- **Deploy tuntas (dieksekusi user, 16 Juli 2026):** panduan step-by-step dipindah dari `README.md` ke `DEPLOY.md` terpisah (10 tahap: Turso DB → push GitHub → import Vercel → env vars → deploy → migrate ke Turso → buat admin production → custom domain → update `NEXT_PUBLIC_SITE_URL` → testing akhir). Hasil akhir: live di **https://dzakydev.my.id**, project Vercel `personal-portofolio-76js` (team `jaee`), DB Turso `personal-portofolio-monn01.aws-ap-northeast-1.turso.io`, auto-deploy dari GitHub `monn01/personal-portofolio` branch `main`. Detail infra tersimpan di memory (`deployment_infra`) untuk sesi mendatang
...
## Changelog rencana
- 16 Juli 2026 — **Tweak visual pasca-live (diminta user setelah deploy production): (1) thumbnail Portfolio diperbesar jadi hero image di Featured Works (Home) & /portfolio (list+detail), (2) preview Certification & Award di Home disederhanakan jadi gambar-saja + tombol panah pojok kanan-bawah ke /certifications, (3) wording admin "Achievement" → "Award" untuk konsistensi.**
  - **Portfolio thumbnail lebih "lookable"**: `PortfolioIcon.tsx` dipecah jadi `PortfolioIcon` (glyph SVG polos) + `PortfolioThumbnail` (pengisi container: gambar asli kalau ada `thumbnailUrl`, fallback icon kategori berwarna kalau tidak) — dipakai di `PortfolioCard` (hero `aspect-video` penuh di atas card, dipakai baik di Home "Featured Works" maupun `/portfolio` karena satu komponen yang sama) dan `PortfolioDetail` (hero `aspect-video` besar di atas artikel, gantikan icon kecil pojok yang sebelumnya cuma 56px)
  - **Simplifikasi Certification & Award di Home**: komponen baru `CertAwardPreviewCard.tsx` — gambar besar (`aspect-[4/3]`) sebagai fokus utama, judul + badge panah bulat (mint accent) di overlay gradient pojok kanan-bawah, seluruh card jadi `<Link href="/certifications">` (bukan ke detail per-item — memang tidak ada halaman detail individual). Kalau item (achievement) belum py gambar, fallback icon trophy/sertifikat di background berwarna. **Halaman `/certifications` sendiri sengaja tidak disentuh** (tetap pakai `CertificationCard`/`AchievementCard` penuh info, sesuai instruksi eksplisit user)
  - **Field baru `Achievement.imageUrl`** (migration `add_achievement_image`, opsional — beda dari `Certification.imageUrl` yang wajib, karena Achievement secara desain tidak selalu py sertifikat fisik/gambar) — `AchievementForm` dapat `<ImageUpload>`, `AdminListRow` di list admin ikut tampilkan thumbnail-nya
  - **Wording admin "Achievement" → "Award"**: cuma teks tampilan (sidebar, judul halaman, tombol, pesan error/konfirmasi) yang diubah — route (`/admin/dashboard/achievements`, `/api/achievement`), nama model Prisma, nama komponen (`AchievementForm`, dst) semua **tetap** `Achievement` (perubahan nama internal di luar scope "konsistensi desain" yang diminta, dan berisiko lebih besar tanpa manfaat tambahan)
  - Testing: `npm run build` sempat gagal type-check duluan karena lupa `npx prisma generate` manual setelah migration (pola bug berulang yang sudah dicatat sejak Module 8/19) — fix dengan generate manual, build+lint lulus setelahnya. End-to-end lewat skrip Node: login → create achievement dengan `imageUrl` (201) → gambar muncul benar di admin list DAN preview Home → delete (200) → dikonfirmasi hilang lagi dari Home. Verifikasi tambahan lewat curl: hero image `aspect-video` muncul di `/portfolio`, `/portfolio/[id]`, dan Home; `/certifications` dikonfirmasi tidak berubah (masih render "Awards" seperti sebelumnya)
- 16 Juli 2026 — **Module 7 (deploy production) selesai.** Panduan deploy dipindah dari `README.md` ke file terpisah `DEPLOY.md` (lebih detail, 10 tahap). Website live di **https://dzakydev.my.id** (Vercel project `personal-portofolio-76js` team `jaee`, DB Turso, auto-deploy dari GitHub `monn01/personal-portofolio`). `README.md` disederhanakan — bagian deploy yang duplikat & sudah usang (masih nyebut `git push origin master`, padahal branch-nya `main`) dihapus, diganti pointer ke `DEPLOY.md`. Semua Definition of Done project-level (PROJECT.md) sekarang [x] kecuali verifikasi visual asli di browser (claude-in-chrome belum pernah terhubung sepanjang project)
- 16 Juli 2026 — **Maintenance pasca-V3: 3 perbaikan dari feedback dosen.** (1) Back/swipe-back di mobile sebelumnya mengikuti history browser biasa (bisa reload halaman sebelumnya) — sekarang di-intercept via `popstate` listener (`components/effects/MobileBackToHome.tsx`, mobile-only lewat `matchMedia`) supaya SELALU balik ke Home dari halaman manapun (dikonfirmasi ke user: scope universal, bukan cuma halaman detail). (2) Navbar mobile diganti dari hamburger dropdown (`MobileNav.tsx`, dihapus) jadi bottom tab bar icon-only (`components/MobileBottomNav.tsx`, fixed di bawah, 8 item termasuk Contact) — data nav (href/icon/accent) diekstrak ke `lib/nav.ts` supaya dipakai bareng oleh `SitePillNav` (desktop) tanpa duplikasi; `ScrollToTopButton` digeser posisinya (`bottom-24` di mobile) supaya tidak nabrak bottom bar. (3) Certification: `getCertifications()` diubah urut `asc` (tahun terlama dulu, sebelumnya `desc`); field baru `Certification.featuredOnHome` (migration `add_certification_featured_on_home`, default `true` — biar tidak mengubah tampilan Home yang sudah ada sampai admin sengaja uncheck) dipakai buat filter preview Home, dikelola lewat checkbox di form admin DAN quick-toggle button (`★ Home`/`☆ Home`) langsung di list `/admin/dashboard/certifications` tanpa perlu buka form edit. Testing: `npm run build`+`npm run lint` sukses (5 warning/error pre-existing tidak berubah), end-to-end lewat skrip tsx sekali-pakai (login → PUT featuredOnHome=false → dikonfirmasi hilang dari Home → restore → dikonfirmasi balik tampil), skrip dihapus setelah selesai.
